...
if [[ -z "${BRANCH_CODENAME:-}" ]]; then
    CURRENT_BRANCH="$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "memory-enhanced")"
...
# Derive dynamic localversion string (e.g. "-Pox-Obsidian-0.9" or "-Pox-0.9")
...
# Derive zip package base name dynamically (e.g. "Pox-Obsidian-0.9-begonia")
...
build_kernel() {
    log "================================================="
    log "Building $KERNEL_NAME ${KERNEL_VERSION:+$KERNEL_VERSION }[$BRANCH_CODENAME]"
    log "Edition:      $BRANCH_CODENAME ($BRANCH_DESC)"
